...
public class Legacy4JSettingsScreen extends OptionsScreen implements TabList.Access {
    ...
    protected void addOptionSection(OptionsScreen.Section section) {
        ...
        if (section == Section.GAME_OPTIONS)
            getRenderableVList().addRenderables(Button.builder(Component.translatable("controls.keybinds.title"), button -> FactoryAPIClient.setScreen(new LegacyKeyMappingScreen(this))).build(), Button.builder(Component.translatable("legacy.options.selectedController"), button -> FactoryAPIClient.setScreen(new ControllerMappingScreen(this))).build());
