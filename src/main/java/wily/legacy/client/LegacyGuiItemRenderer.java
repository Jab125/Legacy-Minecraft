...
public class LegacyGuiItemRenderer implements AutoCloseable {
    ...
    private void createAtlasTextures(int i) {
        GpuDevice gpuDevice = RenderSystem.getDevice();
        this.itemsAtlas = gpuDevice.createTexture("UI items atlas", GpuTexture.USAGE_COPY_DST | GpuTexture.USAGE_TEXTURE_BINDING | GpuTexture.USAGE_RENDER_ATTACHMENT, GpuFormat.RGBA8_UNORM, i, i, 1, 1);
        this.itemsAtlasView = gpuDevice.createTextureView(this.itemsAtlas);
    ...
    public void prepareItemElements(FeatureRenderDispatcher dispatcher,
                                    //? if <26.2 {
                                    /*SubmitNodeStorage submitNodeCollector,
                                    *///?}
                                    GuiRenderState renderState, int frameNumber) {
        if (!renderState.getItemModelIdentities().isEmpty()) {
            int i = this.getGuiScaleInvalidatingItemAtlasIfChanged();
            int j = size * i;
            int k = this.calculateAtlasSizeInPixels(renderState, j);
                                ...
                                if (guiItemRenderState.oversizedItemBounds() != null) {
                                    ...
                                    if (atlasPosition == null || trackingItemStackRenderState.isAnimated() && atlasPosition.lastAnimatedOnFrame != frameNumber) {
                                        ...
                                        if (!bl && this.itemAtlasY + j > k) {
                                            ...
                                            int l = bl ? atlasPosition.y : this.itemAtlasY;
                                            if (bl) {
                                                RenderSystem.getDevice().createCommandEncoder().clearColorAndDepthTextures(this.itemsAtlas, GuiRenderer.CLEAR_COLOR, this.itemsAtlasDepth, 1.0, kx, k - l - j, j, j);
                                            }

                                            this.renderItemToAtlas(dispatcher, submitNodeCollector, trackingItemStackRenderState, poseStack, kx, l, j);
                                            float f = (float) kx / k;
                                            float g = (float) (k - l) / k;
                                            this.submitBlitFromItemAtlas(renderState, guiItemRenderState, f, g, j, k);
                                            if (bl) {
                        ...
                        .forEachItem(
                                guiItemRenderState -> {
                                    ...
                                    if (guiItemRenderState.oversizedItemBounds() != null) {
                                        TrackingItemStackRenderState trackingItemStackRenderState = guiItemRenderState.itemStackRenderState();
                                        OversizedItemRenderer oversizedItemRenderer = oversizedItemRenderers
                                                .computeIfAbsent(trackingItemStackRenderState.getModelIdentity(), object -> new OversizedItemRenderer());
                                        ScreenRectangle screenRectangle = guiItemRenderState.oversizedItemBounds();
                                        OversizedItemRenderState oversizedItemRenderState = new OversizedItemRenderState(
                                                guiItemRenderState, screenRectangle.left(), screenRectangle.top(), screenRectangle.right(), screenRectangle.bottom()
                                        );
                                        oversizedItemRenderer.prepare(oversizedItemRenderState, renderState, dispatcher, i);
                                    }
                                }
                        );
            }
...
//        }
    ...
    private void renderItemToAtlas(FeatureRenderDispatcher featureRenderDispatcher, SubmitNodeStorage submitNodeCollector, TrackingItemStackRenderState trackingItemStackRenderState, PoseStack poseStack, int x, int y, int k) {
        poseStack.pushPose();
        poseStack.translate(x + k / 2.0F, y + k / 2.0F, 0.0F);
        poseStack.scale(k, -k, k);
        boolean bl = !trackingItemStackRenderState.usesBlockLight();
        ...
        if (opacity != 1.0f && !LegacyOptions.enhancedItemTranslucency.get())
            LegacyFeatureRenderDispatcher.of(featureRenderDispatcher).setMap((a, renderType) -> {

                if (renderType == Sheets.cutoutBlockItemSheet()) return Sheets.translucentBlockItemSheet(); // TODO 26.2 it was cutoutBlockSheet before
                else if (renderType.format() == DefaultVertexFormat.ENTITY && !((RenderSetupAccessor)(Object) ((RenderTypeAccessor) renderType).getState()).getTextureBindings().isEmpty())
                    return RenderTypes.entityTranslucentCullItemTarget(((RenderSetupAccessor)(Object) ((RenderTypeAccessor) renderType).getState()).getTextureBindings().get("Sampler0").location());
                return renderType;
            });
        RenderSystem.enableScissorForRenderTypeDraws(x, this.itemsAtlas.getHeight(0) - y - k, k, k);
        trackingItemStackRenderState.submit(poseStack, submitNodeCollector, 15728880, OverlayTexture.NO_OVERLAY, 0);
        ...
        //bufferSource.endBatch();
        LegacyFeatureRenderDispatcher.of(featureRenderDispatcher).setMap((_, rt) -> rt);
        RenderSystem.disableScissorForRenderTypeDraws();
        poseStack.popPose();
    }
