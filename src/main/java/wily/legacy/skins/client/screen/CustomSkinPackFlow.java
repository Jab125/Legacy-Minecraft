...
final class CustomSkinPackFlow {
    ...
    void openOptionsScreen() {
        ...
        FactoryAPIClient.setScreen(new ConfirmationScreen(screen, ConfirmationScreen::getPanelWidth,
    ...
    void editSelectedSkin() {
        ...
        FactoryAPIClient.setScreen(ImportCustomSkinScreen.edit(screen, screen.rootParentScreen(), packId, skin.id(), skin.name(), theme, poses, savedSkinId -> queueRefresh(packId, savedSkinId)));
        screen.playPressSound();
    }
    ...
    private void openCreateScreen(Screen rootParent) {
        if (screen.minecraft != null) FactoryAPIClient.setScreen(CreateCustomSkinPackScreen.create(screen, rootParent));
    }
    ...
    private void openEditScreen(Screen rootParent, String packId) {
        ...
        FactoryAPIClient.setScreen(CreateCustomSkinPackScreen.edit(screen, rootParent, packId, pack == null ? packId : pack.name()));
    }
    ...
    private void openRemoveScreen(Component title, Component message, Runnable action) {
        if (screen.minecraft == null) return;
        FactoryAPIClient.setScreen(new ConfirmationScreen(screen, ConfirmationScreen::getPanelWidth, ConfirmationScreen::getBaseHeight, title, message, s -> action.run()) {
            @Override
            protected void addButtons() {
